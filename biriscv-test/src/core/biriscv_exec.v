...
(
    ...
    ,input  [ 31:0]  opcode_opcode_i     //指令
    ,input  [ 31:0]  opcode_pc_i         //pc
    ,input           opcode_invalid_i
    ,input  [  4:0]  opcode_rd_idx_i
    ,input  [  4:0]  opcode_ra_idx_i
    ,input  [  4:0]  opcode_rb_idx_i
    ,input  [ 31:0]  opcode_ra_operand_i //寄存器值
    ,input  [ 31:0]  opcode_rb_operand_i //寄存器值
    ,input           hold_i
...
begin
    ...
    //x1寄存器被约定为返回地址寄存器
    //call指令：将返回地址存入x1，然后跳转
    //ret指令：从x1读取返回地址并跳转

    if ((opcode_opcode_i & `INST_JAL_MASK) == `INST_JAL) // jal  直接跳转 且可能是 x1寄存器为1时为call指令
    begin
        branch_r        = 1'b1;
        branch_taken_r  = 1'b1;
    ...
    else if ((opcode_opcode_i & `INST_JALR_MASK) == `INST_JALR) // jalr  寄存器间接跳转 or x1寄存器为1且imm12为0时为ret指令 or否则为call指令
    begin
        branch_r            = 1'b1;
        branch_taken_r      = 1'b1;
