...
(
    ...
    ,output          stall_o   //流水线停滞信号（除法未完成或内存访问未完成）
    ,output          squash_e1_e2_o //清空E1-E2流水线（发生异常时）
...
//-------------------------------------------------------------
// E1 / Address //执行阶段1，ALU运算，csr读取，早期异常检测
//------------------------------------------------------------- 
`define PCINFO_W     10
`define PCINFO_ALU       0
...
`define RD_IDX_R    11:7 //寄存器-rd字段位置

reg                     valid_e1_q; //指令有效
reg [`PCINFO_W-1:0]     ctrl_e1_q;  //10位控制向量
reg [31:0]              pc_e1_q;    //当前指令pc
reg [31:0]              npc_e1_q;   //下一条指令pc+4
reg [31:0]              opcode_e1_q;//指令操作码
reg [31:0]              operand_ra_e1_q; //源操作数1
reg [31:0]              operand_rb_e1_q; //源操作数2
reg [`EXCEPTION_W-1:0]  exception_e1_q; //异常状态


always @ (posedge clk_i or posedge rst_i)
if (rst_i)
begin
    valid_e1_q      <= 1'b0;         //指令无效
    ctrl_e1_q       <= `PCINFO_W'b0; //所有控制位清零
    pc_e1_q         <= 32'b0;
    npc_e1_q        <= 32'b0;
    opcode_e1_q     <= 32'b0;
...
else if (issue_stall_i)  //如果流水线停滞，保持E1状态不变
    ;
else if ((issue_valid_i && issue_accept_i) && ~(squash_e1_e2_o || squash_e1_e2_i)) //正常流水线，发射阶段有有效指令，发射阶段被接受，E1-E2阶段没有被清空
begin
    valid_e1_q                  <= 1'b1;
    ctrl_e1_q[`PCINFO_ALU]      <= ~(issue_lsu_i | issue_csr_i | issue_div_i | issue_mul_i); //alu指令是默认类型
    ctrl_e1_q[`PCINFO_LOAD]     <= issue_lsu_i &  issue_rd_valid_i & ~take_interrupt_i; // TODO: Check 有目标寄存器，且不是中断
    ctrl_e1_q[`PCINFO_STORE]    <= issue_lsu_i & ~issue_rd_valid_i & ~take_interrupt_i; //无目标寄存器，且不是中断
    ...
    ctrl_e1_q[`PCINFO_INTR]     <= take_interrupt_i; //中断指令
    ctrl_e1_q[`PCINFO_COMPLETE] <= 1'b1; //所有有效指令都标记为完成

    pc_e1_q         <= issue_pc_i;
    npc_e1_q        <= issue_branch_taken_i ? issue_branch_target_i : issue_pc_i + 32'd4; //如果分支被采取，则使用分支目标地址
    opcode_e1_q     <= issue_opcode_i;
    operand_ra_e1_q <= issue_operand_ra_i;
    operand_rb_e1_q <= issue_operand_rb_i;
    exception_e1_q  <= (|issue_exception_i) ? issue_exception_i :  //异常检测逻辑-前端已经检测到异常-分支地址不对齐异常-无异常
                       branch_misaligned_w  ? `EXCEPTION_MISALIGNED_FETCH : `EXCEPTION_W'b0;
end
// No valid instruction (or pipeline flush event)
else //无有效指令或流水线清空事件
begin
    valid_e1_q      <= 1'b0; //标记指令无效
...
wire   alu_e1_w        = ctrl_e1_q[`PCINFO_ALU];  //没有用
assign load_e1_o       = ctrl_e1_q[`PCINFO_LOAD]; //输出给lsu
assign store_e1_o      = ctrl_e1_q[`PCINFO_STORE];//输出给lsu
wire   csr_e1_w        = ctrl_e1_q[`PCINFO_CSR]; //没有用
wire   div_e1_w        = ctrl_e1_q[`PCINFO_DIV]; //没有用
