...
(
     input                        valid_i          //指令有效
    ,input                        fetch_fault_i    //取指故障标志
    ,input                        enable_muldiv_i  //乘除法拓展使能
    ,input  [31:0]                opcode_i         //指令

    ,output                       invalid_o        //非法指令标志
    ,output                       exec_o           //执行单元使能
    ,output                       lsu_o
    ,output                       branch_o
    ,output                       mul_o
    ,output                       div_o
    ,output                       csr_o
    ,output                       rd_valid_o       //目标寄存器有效标志
);

// 非法指令检测：IMAC指令集内的指令均为合法指令    A：原子操作拓展，提供多核/多线程同步的原子指令 C：压缩指令拓展
...
//目标寄存器有效，排除s指令、分支指令（除jal、jalr）、系统指令
