...
//-----------------------------------------------------------------
//
// biriscv_frontend: Instruction Frontend
//
// Structure:
//   npcg (PC generator) -> npc (branch predictor) -> fetch -> decode
//
// npcg maintains pc_r and generates query address for npc.
// npc outputs are registered (delayed by one cycle).
//
//-----------------------------------------------------------------
...
(
    ...
    ,input           branch_request_i       //更新
    ,input  [ 31:0]  branch_pc_i
    ,input  [  1:0]  branch_priv_i

    ,input           branch_info_request_i  //用于训练，给npc
...
// NPCG outputs
wire  [ 31:0]  npcg_fetch_pc_w;
wire  [  1:0]  npcg_fetch_priv_w;
wire  [  1:0]  npcg_next_taken_w;

wire fetch_allowin;
wire npcg_valid;

//-----------------------------------------------------------------
// NPCG: Next PC Generator
// Maintains pc_r and generates query address for npc
//-----------------------------------------------------------------
biriscv_npcg
#(
...
(
    ...
    ,.info_branch_request_i(branch_info_request_i)
    ,.branch_source_i(branch_info_source_i)
    ,.branch_pc_i(branch_pc_i)

    // NPC interface  (registered output from npc)
    ,.next_pc_f_i(next_pc_f_w)
    ,.next_taken_f_i(next_taken_f_w)

    // Stall interface
    ,.fetch_allowin(fetch_allowin)
    ,.npcg_valid(npcg_valid)

    // Outputs
    // To NPC (query address)
    ,.pc_f_o(fetch_pc_f_w)
    ,.pc_accept_o(fetch_pc_accept_w)

    // To Fetch (current PC with branch mux)
    ,.fetch_pc_o(npcg_fetch_pc_w)
    ,.next_taken_o(npcg_next_taken_w)
);

//-----------------------------------------------------------------
// NPC: Next PC Predictor (Branch Predictor)
// Outputs are registered (delayed by one cycle)
//-----------------------------------------------------------------
biriscv_npc
#(
...
(
    ...
    ,.branch_pc_i(branch_pc_i)
    ,.pc_f_i(fetch_pc_f_w)           // Query address from npcg
    ,.pc_accept_i(fetch_pc_accept_w) // Accept signal from npcg

    ,.mispredicted_is_jmp_r(mispredicted_is_jmp_r)
    ,.mispredicted_is_ret_r(mispredicted_is_ret_r)
    ,.mispredicted_is_call_r(mispredicted_is_call_r)
    ,.mispredicted_is_other_r(mispredicted_is_other_r)
    ,.mispredicted_all_r(mispredicted_all_r)
...
(
    ...
    ,.branch_pc_i(branch_pc_i)
    //分支预测相关-from npcg
    ,.next_pc_f_i(npcg_fetch_pc_w)
    ,.next_taken_f_i(npcg_next_taken_w)
    ,.npcg_valid(npcg_valid)
